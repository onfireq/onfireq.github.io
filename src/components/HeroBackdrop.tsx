...
export default function HeroBackdrop() {
  return (
    <svg className={styles.artwork} viewBox="-500 -500 1000 1000" fill="none" focusable="false">
      ...
      <path stroke="var(--hero-wire)" strokeWidth="1" vectorEffect="non-scaling-stroke" d={frame.edges.map(edge => linePath(edge)).join("")} />
      {frame.rings.map((points, i) => (
        <path key={i} stroke={`var(--hero-ring-${i})`} strokeWidth="1" vectorEffect="non-scaling-stroke" d={linePath(points, true)} />
      ))}
    </svg>
  );
}
